...
export class DlgKit extends KitBase {
    ...
    public fetchDlg<T extends DlgBase>(dlgCtrlType: types_constructor<T>): T {
    ...
    public getDlg<T extends DlgBase>(dlgCtrlType: types_constructor<T>): T {
        for (let index = 0; index < this._dlgList.length; index++) {
            const dlg = this._dlgList[index];
            if (!dlg.isClosed && dlg instanceof dlgCtrlType) {
                return dlg;
    ...
    public closeDlg<T extends DlgBase>(dlgCtrlType: types_constructor<T>) {
